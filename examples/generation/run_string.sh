...
# Force vLLM to regenerate the rollout parquet every run by default. verl's
# main_generation defaults to data.overwrite=False, which silently SKIPS the
# entire vLLM generation step when $SAVE_PATH already exists. On 2026-04-24
# the rlcomps H100 node hit this: stage1 reused a 2.5-month-old rollout from
# Feb 16 and trained stage1 SFT on stale responses (GPU stayed at 0% because
# vLLM never launched). Default = True so "stage1 always rolls out fresh";
# callers who *do* want to reuse a prior rollout can set
# RLCOMP_ROLLOUT_OVERWRITE=False in .env.local or before sourcing rlcomp.sh.
ROLLOUT_OVERWRITE=${RLCOMP_ROLLOUT_OVERWRITE:-True}
...
${PYTHON_BIN} -m verl.trainer.main_generation \
    ...
    data.overwrite=${ROLLOUT_OVERWRITE} \
