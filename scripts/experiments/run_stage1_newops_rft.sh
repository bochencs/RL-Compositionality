...
: "${NPROC_PER_NODE:?NPROC_PER_NODE not set; source scripts/env/bootstrap.sh first}"
...
# DataLoader worker count. bootstrap.sh sets this based on /dev/shm probe:
# 0 on small-shm K8s pods (avoids SIGBUS/Bus error at iter-0), 8 on large-shm
# hosts. Caller can override by exporting SFT_NUM_WORKERS before running.
