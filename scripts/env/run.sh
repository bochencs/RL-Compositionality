...
_pipeline_init_logdir() {
    ...
    # Capture the bootstrap banner and initial env for forensics.
    {
        ...
        env | grep -E "^(RLCOMP_|GPU_|MODEL_|ACTOR_|REF_|CPU_|OFFLOAD_|GRAD_|SAVE_|TEST_|N_GPUS|NNODES|NPROC|HF_|TMPDIR|RAY_|WANDB_MODE|WANDB_PROJECT|WANDB_DIR|VIRTUAL_ENV|PYTHONPATH)=" | sort
